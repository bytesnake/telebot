...
pub fn derive_getter(input: TokenStream) -> TokenStream {
    ...
    let expanded = expand_getter(ast);
    expanded.to_string().parse().unwrap()
}

fn expand_getter(ast: syn::MacroInput) -> quote::Tokens {
...
#[proc_macro_derive(TelegramSendable)]
pub fn derive_telegram_sendable(input: TokenStream) -> TokenStream {
    let ast = syn::parse_macro_input(&input.to_string()).unwrap();
    let expanded = expand_sendable(ast);
    expanded.to_string().parse().unwrap()
}

fn expand_sendable(ast: syn::MacroInput) -> quote::Tokens {
    let config = config_from(&ast.attrs);
